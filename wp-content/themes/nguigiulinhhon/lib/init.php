...
function gs_constants() {
	...
	// Child theme (Change but do not remove)
		...
		define( 'CHILD_THEMES_VERSION', $theme->Version );
		
		/** @type constant Child Theme Name, used in footer. */
		define( 'CHILD_THEMES_NAME', $theme->Name );
		
		/** @type constant Child Theme URL, used in footer. */
		define( 'CHILD_THEMES_URL', $theme->get('ThemeURI') );
...
function gs_init() {
	...
	include_once( CHILD_LIB_DIR . '/functions/snippets.php' );
	...
	if ( is_admin() ) {
		...
		/** Inpost Metaboxes */
		//include_once( CHILD_LIB_DIR . '/plugins/special-post-widget.php');		
